...
survey <- read_csv('https://github.com/byuistats/Math221D_Cannon/raw/master/Data/HighSchoolSeniors_subset.csv') 
```
...
Notice that the mean is `Inf` which means infinity, or undefined.  This is likely due to R attempting to divide a number by zero, meaning someone answered that their arm span was zero.  This is where [`filter()`](../3-Data_Wrangling_Visualization/3-Filter.qmd) comes in handy.  We can filter out the rows where Armspan_cm is 0:
...
1. Click to see how to [filter](../3-Data_Wrangling_Visualization/3-Filter.qmd) outliers of ht_to_wt ratio and [select](../3-Data_Wrangling_Visualization/4-Select.qmd) only the columns of interest.
