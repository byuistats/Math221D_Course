...
This lesson explores the idea of a **sampling distribution** which is the distribution of a sample statistic.  We discuss the theoretical concepts that allow us to quantify our uncertainty and allow us to make confident conclusions about an entire population based on a single sample.  
...
You'll all get different results because you'll be asking different sets of people.  Now, instead of looking at your percents separately, summarize the collection of samples from all 11 of you using `favstats()` and a histogram:  
...
__NOTE__:  The data above is based on a simulation for proportions from 11 independent samples of size 50.  Because it's a simulation, I know what the true proportion is.  


The above histogram represents 11 different samples of nearly infinite possible samples of 50 BYU-I students.  

The __Sampling Distribution__ is the distribution of a sample statistic for *all possible samples* from a population.  It's a theoretical construct, completely imaginary.  But there are some mathematically provable attributes of this theoretical distribution that make it possible to make inference about an unknown population parameter.  

We will discuss these theoretical properties in greater detail below.  But one of the most important properties of sampling distributions is that the Mean of the sampling distribution will, under certain conditions, be very close to the true population parameter.  
...
If you guessed the mean (0.0709) you'd be VERY close to the truth!  Isn't it fascinating that none of the researchers got 7% yet this is the true proportion?  (It's actually impossible to get 7% exactly when only asking 50 students.)  

It turns out that the _mean of the sampling distribution is equal to the population parameter!_  This is true for any sample statistics.  

We not only know about the *mean* of a sampling distribution.  We can know the whole distribution! 
