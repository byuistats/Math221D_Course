...
Because we know that when x-bar is normally distributed, then $Z$ follows a *standard normal* distribution.  We can calculate the probability of getting a test statistic LESS than the one we observed, if the null hypothesis is true.  
...
Every statistical hypothesis test will follow the form: 
  ...
  1. State the null and alternative **hypotheses** and choose the significance level, $\alpha$  
  2. Calculate the **Test Statistic** using your sample data  
...
__NOTE__:  This is a new paradigm for most students.  It's perfectly fine to be confused at first.  But just as with learning to code in R, we will make sense of this framework for statistical inference.  
