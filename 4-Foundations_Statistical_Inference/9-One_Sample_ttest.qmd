...
Recall that when we know what the population standard deviation, $\sigma$, for individuals, and are confident that the distribution of sample means is approximately normal, we can use a Z-score and the Standard Normal Distribution to calculate probabilities.  
...
So far we have been discussing a single, quantitative variable of interest, like test scores, reaction times, heights, etc.  Histograms are very good when looking at one variable at a time.   
