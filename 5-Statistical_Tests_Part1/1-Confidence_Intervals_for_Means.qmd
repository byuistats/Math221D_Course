...
The statistics above are called **point estimators** because they are just one number (one point on a number line) that is used to estimate a population parameter.  Parameters are generally unknown values.  Think about the mean.  If $\mu$ is unknown, how do we know if $\bar{x}$ is close to it?  
...
Flipping this around, if we take our **sample mean, $\bar{x}$,** and make an interval 2 standard deviations, $2\frac{\sigma}{\sqrt{n}}$ above the mean and 2 below, the interval will overlap with the *true population mean* about 95% of the time.   
