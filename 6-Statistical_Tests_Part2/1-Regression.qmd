...
If our simple linear model adequately explains the relationship between $x$ and $y$, then the residuals well have certain properties.  Ideally, the residuals:

    1. Are normally distributed
    2. Have constant variance across the range of predicted values

We will show how to evaluate these when introducing the test requirements for a statistical hypothesis test below. 
