...
The Test Statistic for comparing >2 group means is called $F$.  
...
__NOISE__:  The variation *within* groups.  This represents the natural variation inherent in the data independent of any differences between the means.  

The example below will illustrate what we mean by this and why this is a suitable test statistic for comparing differences between means.  
...
The null hypothesis is that all 4 pesticides are all equally effective, meaning the average emergence is the same regardless of which treatment you use.  
...
To get more data, we would test each seed treatment on multiple farms.  Ideally, we'd want a good random sample of farms in the area we expect to be planting.  
...
If the null hypothesis is *false*, we would expect the group means to be much more spread out than the within-treatment variation.  
...
The probability distribution of the F-statistic changes shape depending on how much data we have AND how many groups we have.  These parameters in the $F$-distribution are called **Degrees of Freedom**, and the F-distribution has 2 sets.  
