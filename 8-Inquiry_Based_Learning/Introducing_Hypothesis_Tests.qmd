...
The exact formula for a test statistic depends on what type of analysis we are performing.  But regardless of the type of analysis, test statistics follow a general pattern.  
...
As we get into specific analyses, we will look at a few of those formulas, but R will do all the calculations for us.  For now, consider a test statistic as a signal-to-noise ratio.  The larger it is, the stronger the evidence against the null hypothesis.  
