...
Statistical hypothesis testing is about calculating the **probability** of observing what we actually observed **IF the null hypothesis were true**.  If that probability if very small, we can confidently reject the null hypothesis is true in favor of the alternative.  
...
Every test statistic that we will use in this class has a probability distribution associated with it.  The shape of that distribution will depend on context, but the general process is:  
