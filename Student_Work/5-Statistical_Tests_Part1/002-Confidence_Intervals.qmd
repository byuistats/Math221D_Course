...
By the end of this lesson, you should be able to:
  ...
  2. Explain the meaning of a level of confidence
  3. Create a confidence interval for a single mean with $\sigma$ known using the following steps:
  ...
  4. Explain how the margin of error is affected by the sample size and level of confidence
...
ggplot(df, aes(x = x, y = y)) +
  ...
  labs(title = "Standard Normal Distribution",
       subtitle = "~95% of the Xbars will be within 2 Standard Deviations from the mean",
       x = "",
       y = "Density") +
  theme_minimal() 
```
